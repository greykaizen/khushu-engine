...
|---|---|---|
| core | ⬜ planned | `core/astronomy/AstroTime.kt`, `CalendarHelper.parseTimeZone/epochForLocalHour` |
| astronomy | 🟨 porting (largest donor overlap) | `AstroMath.kt`, `AstroEphemerisEngine.kt`, `AstroEventEngine.kt`, `AstroSchedule.kt`, `LunarOrientation.kt`, `feature/calendar/SunMoonCalculator.kt`, `feature/astro/solar/SolarEphemeris.kt` |
...
| prayer | ⬜ planned | `domain/prayer/PrayerCalculation.kt`, `CalendarHelper.calculatePrayers/getPrayerBadgeInfo` |
| qibla | ⬜ planned (tiny) | bearing math currently embedded in Qibla feature |
| zakat | ⬜ planned | logic embedded in `feature/zakat/ZakatAlMalCalculator.kt`, `FitranaCalculator.kt` — extract, don't copy UI |
| cli | ⬜ planned | — |
| goldens | ⬜ pending dump | generate from Osprey before it drifts |
