...
| Typed `Kilometers`-at-boundary retypes (`travelFacts`, `passagesOver` raw Doubles) + dead-API removals (`GOLDEN_HOUR_MORNING_START`, `Radians`, `blueHourLowerDeg`, `HilalReport.moonAgeHours` nullable) | ⬜ breaking → 2.0.0 | host-integration milestone — Khushu host app (WIP) is the trigger; schedule the 2.0.0 cleanup pass when it integrates |
