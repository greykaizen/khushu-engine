...
| astronomy | ✅ locked | cosinekitty 2.1.19 (JitPack) behind capability API; sun/moon/hilal + one-pass `moon.track` + eclipses/distance-extremes; 576-case golden matrix green; divergences D7-D8; v1.13: epoch-envelope + boundary validation (phaseName NaN, track flag-independence, subsolar targets), cosinekitty wrapped in typed UpstreamComputationException, `sun.events` reads the pinned AltitudeConventions table, hilal no-fabrication guard (D22), facade `seasons`/`sun.audit` delegated |
| calendar | ✅ locked | ummalqura-calendar 2.0.2 behind capability API; hijri/events/fastDays + `date`/`month`/`facts`/`events(registry)`/`observance`/`lunar` namespaces; out-of-table dates throw typed `UpstreamComputationException`; D10-D11; v1.8: seek-nights+Tashreeq unified into `builtInDefinitions` (events()↔registry agree), offset-correct definition-driven `upcomingEvents`, `daysUntil` countdowns, `HijriDate : Comparable`; v1.9: regional civil calendars (`CivilCalendarType` ×9, `RegionalCalendars`, `DateLine.Regional`), `previousOccurrence`/`daysSince`, `month.summary` composite; v1.15: sighted-calendar mode (`SightedCalendar` — announcement-anchored months, `localOffsetDays` global correction, tabular fallback, contradiction rejection, events under sighted derivation) |
| prayer | ✅ locked | wraps `com.batoulapps.adhan:adhan2-jvm:0.0.7`; 6184-case golden matrix green; polar-safe; + `stats.streak` (delegates to core DayStreakMath in 2.0.0), `nextOccurrenceOf`, `nextJumuah`, `nightDivisions`, `fastingFactsForMonth`; v1.17: `qadaReport` (ḥayḍ/nifās boundary obligations — pairing per school, Ḥanafī witr, ghusl times) + `ANTI_TRANSIT_FAJR` persistent-twilight rule; see docs/divergences.md D1–D4 |
| qibla | ✅ locked | bearing + distance owned by engine; shadow-verification + relative sun angle + `audit()` provenance; cross-checked vs adhan2 (docs/divergences.md D5); v1.10: `relativeSunAngle` moved into module (facade delegates), shadow-cache in CachedEngine; v1.13: Kaaba-antipode longitude normalization bug fixed (D21, caught by the new subsolar validation) |
| zakat | ✅ locked | mal + fitrana + hawl + livestock schedules + ushr + rikaz; fiqh spot cases + property tests; rate 0.025, madhab rules, saʿ conventions documented; v1.10: dual nisab thresholds, madhab-rule `notes`, `AssetKey` i18n keys, `totalKg`, `hawlFacts` countdown; v1.11: deferred-queue topics — `DebtTreatment` policies (IMMEDIATE_ONLY Hanafi default per Daruliftaa #8395), receivable tiers (strong/medium/weak), `ShareHolding` (AAOIFI Std 35 intent/basis), jewelry realizable-value quotes, mixed-irrigation PREDOMINANT (al-Ikhtiyar) vs PROPORTIONAL alternative, `UshrNisabPolicy` + documented 5-wasaq conversion, `FitrPaymentMode` provenance; see `docs/v1.11-zakat-design.md` |
| mushaf | ✅ locked | glyph-atlas layout math ported from donor QuranApp rasterizer/measurer; `Mushaf` capability (fit/measure/layout); golden + property tests green; atlas SPEC DATA lives in khushu-data-api (`inventory/atlas/`), engine stays content-free; v1.12: typed `InvalidParameterException` validation (unitsPerEm/ppem > 0, finite placements, finite fontSize/gap/width), NaN/Infinity guards at every entry point, `skippedGlyphIds` reporting on WordLayout/AyahLayout (missing glyphs advance pen per donor parity but are now diagnosable), `engine:core` dependency added for the shared error model |
| facade | ✅ locked | thin delegation namespaces (`engine.astronomy.sun.*` etc.), zero logic; KDoc on all public members; `DayApi.summary` composite + `CachedEngine` LRU decorator (prayer + calendar + astronomy + qibla caches, forceRecompute overwrite, per-domain clear); v1.16: `engine.tasbih.*` + `engine.observance.*` delegation |
| tasbih | ✅ locked | dhikr counting domain: 7 canonical engine-seeded presets (hadith-cited KDoc — D1), factories over data-api duas/99-Names (`ofDua`/`ofName` — "select → create tasbih" flow), STRICT daily-goal streaks via core DayStreakMath, per-definition stats; host persists definitions+logs, engine stateless; design `docs/v1.16-tasbih-observance-design.md` |
| observance | ✅ locked | centralized bookmarks (single system for quran/sunnah/dua/names/articles — opaque refs validated by host via data-api, toggle/upsert/dedup/anchor-merge, grouped/recent views) + reading-session stats (explicit-mark STRICT streaks, targets, continue-reading, history) + khatm math over host-supplied juz/range tables (data-api `quran_metadata` is the single source; engine embeds only `TOTAL_AYAHS = 6236` as a provenance-default) + shared core DayStreakMath namespace; design `docs/v1.16-tasbih-observance-design.md` |
| cli | ✅ locked | all spec §7 commands incl. `verify` (both golden suites PASS); not part of public API |
| store | ✅ locked | companion persistence (v1.7.0): `SettingsSnapshot` DTOs covering every tunable, `SettingsCodec` (versioned JSON + migration seam), `KhushuSettingsStore` DataStore binding; outside engine purity lock; observance logs stay host-owned — README §Observance logs / §Disk caching guidance; v1.8: calendar display sides persisted (primary/secondary); v1.9: `civilCalendar` side persisted |
...
| Item | Status | Gate |
|---|---|---|
| Fajr-at-anti-transit persistent-twilight policy | ✅ shipped v1.17.0 as `HighLatitudeRule.ANTI_TRANSIT_FAJR` (opt-in; Muwaqqit-documented position per `docs/aqrab-al-ayyam-review.md`; engine substitutes `sun.antiTransit` when persistent twilight detected for Fajr angle; KDoc carries provenance — host verifies per their fiqh) | none — parameter model, not a gate |
| Host-app transplant (Osprey AR) | ⬜ deferred by user decision | `docs/ar-transplant.md` is the seam contract when resumed; Osprey is DONOR not host — the AR transplant's consumer is the future Khushu host app (WIP), not Osprey |
| Qada/excused-range helpers | ✅ shipped v1.17.0 as parameterized positions (design `docs/qada-design.md` rev 3 — gate dissolved by parameter model: engine implements documented school positions, host selects, provenance in KDoc + result notes; Darul Iftaa #8301 closes granularity cell) | primary-text pulls remain a KDoc flag — corrections ship as parameter-default patches |
...
| Typed `Kilometers`-at-boundary retypes (`travelFacts`, `passagesOver` raw Doubles) + dead-API removals (`GOLDEN_HOUR_MORNING_START`, `Radians`, `blueHourLowerDeg`, `HilalReport.moonAgeHours` nullable) | ✅ shipped 2.0.0 (breaking cleanup — all raw Doubles at public boundaries now typed; dead APIs removed; moonAgeHours nullable per D7; qibla normalizeDeg extracted to core AngleMath; prayer delegates to core DayStreakMath) | none — complete |
...
| KMP targets, recitation audio mirroring, adhan2↔cosinekitty rise/set unification | ✅ resolved/closed | KMP: Android-only suffices (JVM artifacts serve it — no migration needed unless iOS is wanted). Recitation audio: external CDN by design (no hosting decision to make). Rise/set unification: no user-facing value (adhan2 is correct for prayer times; cosinekitty is correct for astronomy facts) — YAGNI, dropped |
